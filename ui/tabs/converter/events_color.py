...
def bind_color_events(components, states, theme_state):
    ...
    def build_palette_html_with_selection(
        cache,
        replacement_regions,
    ...
    def on_palette_row_select(
        row_id, selected_user_row_id, selected_auto_row_id, cache
    ):
    ...
    states["conv_lut_color_trigger_btn"].click(
        ...
        outputs=[
            states["conv_replacement_color_state"],
    ...
    states["conv_palette_delete_trigger_btn"].click(
        ...
        inputs=[
            states["conv_preview_cache"],
            states["conv_replacement_regions"],
            states["conv_replacement_history"],
            states["conv_selected_user_row_id"],
            states["conv_loop_pos"],
            components["checkbox_conv_loop_enable"],
            components["slider_conv_loop_width"],
    ...
    ).then(fn=lambda: None, inputs=[], outputs=[states["conv_selected_auto_row_id"]])
    ...
    def on_color_picker_find_nearest(picker_hex, lut_path):
        ...
        try:
            ...
            gr.Info(
                f"\u2705 \u6700\u63a5\u8fd1: {nearest_hex} (\u8ddd\u79bb: {dist[0]:.1f})"
            )
    ...
    def _render_free_color_html(free_set):
        if not free_set:
            return ""
        parts = [
            "<div class='flex-center gap-sm' style='flex-wrap:wrap; padding:4px;'>",
            "<span class='text-caption'>\U0001f3af 自由色:</span>",
        ]
        for hex_c in sorted(free_set):
            parts.append(
                f"<div class='lut-color-swatch swatch-sm free-color-mark' "
                f"style='background:{hex_c};' title='{hex_c}'></div>"
            )
        parts.append("</div>")
        return "".join(parts)
    ...
    components["btn_conv_free_color"].click(
        ...
        outputs=[
            states["conv_free_color_set"],
            states["conv_free_color_html"],
            components["textbox_conv_status"],
        ],
    )
    ...
    def on_preview_click_sync_ui(cache, evt: gr.SelectData, lut_path):
        from ui.widgets.palette import (
    ...
    # ------------------------------------------------------------------
    def on_color_selected_for_relief(
        hex_color, enable_relief, height_map, base_thickness, cache
    ):
        ...
        matched_hex = (
            (cache or {}).get("selected_matched_hex", hex_color) if cache else hex_color
        )
