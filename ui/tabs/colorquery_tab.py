...
def create_5color_tab_v2(lang="zh"):
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            status_text = gr.Textbox(
                label="状态", value="💡 请选择 LUT 文件", interactive=False, lines=2
            )

            sequence_text = gr.Textbox(
                label="选择序列", value="", interactive=False, lines=2
            )
        ...
        with gr.Column(scale=2):
            ...
            # 使用 HTML 显示颜色色块
            colors_html = gr.HTML(
                value=_empty_colors_html(), elem_id="colors-html-5color"
            )
            ...
            for i in range(10):
                btn = gr.Button(
                    f"Color {i}",
                    visible=True,
                    elem_id=f"color-btn-{i}-5color",
                    elem_classes=["hidden-5color-btn"],
                )
    ...
    def on_load(lut_path):
        ...
        try:
            ...
            if lut_path.endswith(".npz"):
                ...
                # 检测颜色数量
                color_count, combo_count = ColorCountDetector.detect_color_count(
                    rgb_data
                )

                if color_count == 0:
                    return (
                        f"❌ 无法识别的 LUT 格式（{combo_count} 个组合）",
                        "",
                        _empty_colors_html(),
                        _empty_result(),
                        "",
            ...
            # 生成颜色 HTML
            colors_html_content = _generate_colors_html_v2(
                base_colors, color_count, color_names
            )
    ...
    def on_color_select(color_idx, lut_path, selected_json, color_names_json):
        ...
        if len(selected) >= 5:
            return (
                selected_json,
                _format_seq(selected, color_names),
                _error_result("已选择 5 个颜色"),
            )
    ...
    undo_btn.click(
        fn=on_undo,
        inputs=[selected_state, color_names_state],
        outputs=[selected_state, sequence_text, result_html],
    )
    reverse_btn.click(
        fn=on_reverse,
        inputs=[selected_state, color_names_state],
        outputs=[selected_state, sequence_text],
    )
    query_btn.click(
        fn=on_query, inputs=[lut_path_state, selected_state], outputs=[result_html]
    )
...
def _empty_colors_html():
    return '<div class="dashed-box" style="min-height:200px;display:flex;align-items:center;justify-content:center;">请先选择 LUT 文件</div>'
...
def _generate_colors_html_v2(base_colors, color_count=None, color_names=None):
    ...
    html = f'<div class="color-grid-v2" id="color-grid-5color-v2" style="grid-template-columns: repeat({columns}, 1fr); gap: 15px;">'

    for idx, rgb in enumerate(base_colors):
        hex_color = rgb_to_hex(rgb)
        color_name = (
            color_names[idx] if color_names and idx < len(color_names) else f"色{idx}"
        )
        html += f"""
...
def _empty_result():
    return '<div class="dashed-box">选择 5 次颜色后点击查询</div>'


def _error_result(msg):
    return f'<div class="msg-card msg-card-error">❌ {msg}</div>'
...
def _result_html(result):
    ...
    return f"""
    <div class="msg-card msg-card-success">
        <div style="text-align:center;margin-bottom:15px;">
            <div class="msg-title">✅ 查询成功{mode_text}</div>
            <div class="msg-detail">行索引: {result.row_index}</div>
        </div>
        <div class="flex-center gap-xl" style="justify-content:center;">
            <div class="color-result-swatch" style="background-color:{hex_color};"></div>
