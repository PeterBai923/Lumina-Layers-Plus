...
### 新功能
- **变量层高变换** - 支持基于模型几何的动态层高调整，优化打印质量和时间
- **K-Means 种子配置** - 可配置的 K-Means 初始化种子，实现可复现的颜色聚类结果
- **统一日志模块** - 集中式日志系统，支持结构化输出和可配置日志级别

### 性能优化
- **多模块 GPU 加速** - 优化跨多个处理模块的 GPU 加速，提升图像转换速度
- **向量化立方体顶点/面生成** - 用向量化 NumPy 操作替换迭代式网格生成，显著提升网格创建速度
- **增强聚类空间特征** - 改进 GPU K-Means 聚类的空间特征支持，实现更准确的颜色分割
- **优化网格膨胀** - 改进网格膨胀迭代和收缩偏移参数处理

### 代码重构
- **模块化架构重组** - 将代码库重组为逻辑模块（color、image、lut、mesh、preview、utils），职责清晰
- **CUDA 支持集成** - 添加原生 CUDA 支持，支持 GPU 加速操作和自动设备检测
- **工具模块迁移** - 将工具模块迁移至 core 目录，优化代码组织
- **简化 NumPy 补丁** - 移除日志分流并简化 NumPy 补丁处理
- **功能移除** - 移除掐丝珐琅模式、透明镀层、分段 GLB 预览、像素/矢量建模模式，精简代码库
...
### 新功能
- **背板层高设置** - 支持为背板设置独立的层高（默认 0.2mm），与彩色层（0.08mm）区分，减少打印时间
...
### 代码重构
- UI 组件从 Markdown 迁移到 HTML
- 移除国际化模块，统一使用中文
- CSS 重构为模块化文件结构
- converter_tab 拆分为模块化子包
- 各标签页（校准、合并、颜色提取器）拆分为独立模块
- 移除动态热床尺寸选择功能
- 移除系统托盘代码
- 清理调试输出和冗余注释
