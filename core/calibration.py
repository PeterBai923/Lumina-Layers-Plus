...
def get_top_1296_colors():
    ...
    """
    logger.info("[SMART] 正在模拟 6^5 = 7776 种组合...")
    ...
    logger.info("[SMART] 总候选数: %s。正在筛选前 1296 个...", len(candidates))
    ...
    logger.info("[SMART] 种子颜色: %s", len(selected))
    ...
    logger.info("[SMART] 第一轮（高质量）已选择: %s", len(selected))
    
    # Round 2: Fill remaining slots with lower threshold
    if len(selected) < target:
        logger.info("[SMART] 正在填充剩余 %s 个位置...", target - len(selected))
        for c in candidates:
            if len(selected) >= target:
                break
            if any(c['stack'] == s['stack'] for s in selected):
    ...
    logger.info("[SMART] 最终选择: %s 种颜色", len(selected))
...
def generate_smart_board(block_size_mm=5.0, gap_mm=0.8):
    ...
    Returns:
        Tuple of (output_path, preview_image, status_message)
    """
    logger.info("[SMART] 正在生成 Smart 1296 校准板（38x38 布局）...")
    ...
    logger.info("[SMART] 板尺寸: %.1f x %.1f mm（网格: %sx%s）", board_w, board_h, total_dim, total_dim)
    ...
    logger.info("[SMART] Voxel 矩阵: %s x %s x %s", total_layers, voxel_h, voxel_w)
    ...
    logger.info("[SMART] [OK] Smart 1296 板已生成: %s", output_path)
...
def generate_8color_board(page_index=0):
    ...
    try:
        ...
        logger.info("[8COLOR] 已从 %s 加载 %s 个堆叠", path, len(all_stacks))
        ...
        logger.error("[8COLOR] 加载数据错误: %s", e)
...
def generate_bw_calibration_board(block_size_mm=5.0, gap_mm=0.8, backing_color="White"):
    ...
    Returns:
        Tuple of (output_path, preview_image, status_message)
    """
    logger.info("[BW] 正在生成黑白校准板（8x8 布局）...")
    ...
    logger.info("[BW] 板尺寸: %.1f x %.1f mm（网格: %sx%s）", board_w, board_h, total_dim, total_dim)
    ...
    logger.info("[BW] Voxel 矩阵: %s x %s x %s", total_layers, voxel_h, voxel_w)
    
    # Generate all 32 combinations (2^5 = 32)
    logger.info("[BW] 正在生成 32 种组合（2^5）...")
    stacks = []
    for i in range(32):
    ...
    logger.info("[BW] [OK] 黑白校准板已生成: %s", output_path)
...
def select_extended_1444_colors(base_1024_stacks):
    ...
    """
    logger.info("[5C_EXT] 正在从 3073 个候选中选择 1444 个扩展颜色...")
    ...
    logger.info("[5C_EXT] 总候选数: %s（3072 + 1 KWWWWW）", len(candidates))
    ...
    logger.info("[5C_EXT] 已预选 KWWWWW（特殊情况）")

    target = 1444

    logger.info("[5C_EXT] 第一轮：贪心选择（RGB 距离 > 8）...")
    ...
    logger.info("[5C_EXT] 第一轮已选择: %s", len(selected))

    if len(selected) < target:
        logger.info("[5C_EXT] 正在填充剩余 %s 个位置...", target - len(selected))
        for c in candidates:
            if len(selected) >= target:
                break
            if any(c['stack'] == s['stack'] for s in selected):
    ...
    logger.info("[5C_EXT] 最终选择: %s 种颜色", len(selected))
...
def generate_5color_extended_board(block_size_mm=5.0, gap_mm=0.8, page_index=0):
    ...
    Returns:
        Tuple of (output_path, preview_image, status_message)
    """
    logger.info("[5C_EXT] 正在生成 5 色扩展校准板 - 第 %s 页...", page_index + 1)
...
def _generate_5color_base_page(block_size_mm, gap_mm, preview_colors, slot_names):
    ...
    logger.info("[5C_EXT] 正在生成基础页（1024 色，5 层）...")
...
def _generate_5color_extended_page(block_size_mm, gap_mm, preview_colors, slot_names):
    ...
    """
    logger.info("[5C_EXT] 正在生成扩展页（1444 色，6 层）...")
